...
class Interactor:
    def __init__(self, cfg, table, reqs, proc, trace=False, timeout=30.0):
        ...
        # Занятость ресурсов — чтобы видеть, что на самом деле узкое место.
        self.busy_time = {k: 0.0 for k in self.busy}
        self.link_time = {"UP": 0.0, "DOWN": 0.0}
        self.heap = []
        self.seq = 0
        self.now = 0.0
        self.finished = 0
    ...
    def queue_transfer(self, t, direction, remote, length, kind, rids):
        ...
        self.link_time[direction] += self.transfer_ms(length)
    ...
    def occupy(self, server, t, dur, spec, meta, used):
        used.add(server)
        self.busy_time[server] += self.S + dur
    ...
    def utilization(self):
        span = max(1e-9, max(self.link_free.values()) )
        out = {}
        out["E"] = self.busy_time["E"] / span
        ck = [self.busy_time["C%d" % k] / span for k in range(self.K)]
        out["C_avg"] = sum(ck) / len(ck)
        out["C_max"] = max(ck)
        out["UP"] = self.link_time["UP"] / span
        out["DOWN"] = self.link_time["DOWN"] / span
        return out
...
def main():
    ...
    u = res["util"]
    out.write("util_E %.3f\n" % u["E"])
    out.write("util_C_avg %.3f\n" % u["C_avg"])
    out.write("util_C_max %.3f\n" % u["C_max"])
    out.write("util_UP %.3f\n" % u["UP"])
    out.write("util_DOWN %.3f\n" % u["DOWN"])
    out.write("score %.3f\n" % res["score"])
    out.flush()
